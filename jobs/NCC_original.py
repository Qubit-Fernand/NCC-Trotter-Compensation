...
def main():
    ...
    def compensation_unitary(W, theta, atol=1e-10):
        ...
        if antihermitian_err <= atol:
            return expm(theta * W)
        raise ValueError(
            f"sampled W is neither Hermitian nor anti-Hermitian "
            f"(herm_err={hermitian_err:.3e}, antiherm_err={antihermitian_err:.3e})"
        )
    ...
    def NCC_sampling(trials):
        ...
        for _ in tqdm(range(trials), desc="single step trails"):
            ...
            if s == 3:
                ...
                P2 = rng.choice([XX_term(j2), YY_term(j2), ZZ_term(j2), Z_term(j2)], p=p_Pauli)
                b2 = rng.choice([0, 1])
