...
# This script parallelizes independent repetitions with threads so expensive
# evolution/basis caches remain shared across repeats. Static/evolution cache
# construction is allowed to use the backend's normal BLAS threading; only the
# per-repeat sampling section is optionally BLAS-limited to avoid oversubscription.
...
except ImportError:  # pragma: no cover - handled at runtime on minimal installs
...
BLAS_PHASE_CONDITION = threading.Condition()
ACTIVE_SAMPLERS = 0
WAITING_CACHE_BUILDERS = 0
ACTIVE_CACHE_BUILDERS = 0
...
def resolved_channel_parameters(args):
    if args.mode == "original":
        return None, None
...
def build_static_for_case(args, mode_impl, q0, s0):
    if args.mode == "original":
        return mode_impl.build_static_data(n=args.N, j=args.J, h=args.h)
    return mode_impl.build_static_data(n=args.N, q0=q0, s0=s0, j=args.J, h=args.h, K=1)


def run_without_active_sampling(func):
    global WAITING_CACHE_BUILDERS, ACTIVE_CACHE_BUILDERS
    with BLAS_PHASE_CONDITION:
        WAITING_CACHE_BUILDERS += 1
        BLAS_PHASE_CONDITION.notify_all()
        try:
            while ACTIVE_SAMPLERS:
                BLAS_PHASE_CONDITION.wait()
        finally:
            WAITING_CACHE_BUILDERS -= 1
        ACTIVE_CACHE_BUILDERS += 1
    try:
        return func()
    finally:
        with BLAS_PHASE_CONDITION:
...
def get_evolution_data(
    ...
    if cache_lock is None:
        if cache_key not in evolution_cache:
            evolution_cache[cache_key] = run_without_active_sampling(lambda: mode_impl.build_tilde_V(static, t_total, r))
        return evolution_cache[cache_key]
    with cache_lock:
        if cache_key not in evolution_cache:
            evolution_cache[cache_key] = run_without_active_sampling(lambda: mode_impl.build_tilde_V(static, t_total, r))
        return evolution_cache[cache_key]
...
def ensure_basis_batches_cached(mode_impl, static: dict, evolution_data: dict, r: int, cache_lock=None):
    if cache_lock is None:
        return run_without_active_sampling(lambda: ensure_basis_batches(mode_impl, static, evolution_data, r))
    with cache_lock:
        return run_without_active_sampling(lambda: ensure_basis_batches(mode_impl, static, evolution_data, r))


def blas_sampling_context(blas_threads: int):
    if blas_threads is None or blas_threads <= 0:
        return nullcontext()
    if threadpool_limits is None:
        print(
            "warning: threadpoolctl is not installed; cannot limit BLAS threads during sampling",
            file=sys.stderr,
        )
        return nullcontext()
...
class SamplingBlasContext:
    def __init__(self, blas_threads: int):
        self.blas_threads = blas_threads
        self.thread_context = None

    def __enter__(self):
        global ACTIVE_SAMPLERS
        with BLAS_PHASE_CONDITION:
            while WAITING_CACHE_BUILDERS or ACTIVE_CACHE_BUILDERS:
                BLAS_PHASE_CONDITION.wait()
            ACTIVE_SAMPLERS += 1
        self.thread_context = blas_sampling_context(self.blas_threads)
        return self.thread_context.__enter__()

    def __exit__(self, exc_type, exc, tb):
        global ACTIVE_SAMPLERS
        try:
            return self.thread_context.__exit__(exc_type, exc, tb)
        finally:
            with BLAS_PHASE_CONDITION:
                ACTIVE_SAMPLERS -= 1
                BLAS_PHASE_CONDITION.notify_all()
...
def estimate_total_sample_error(
    ...
    with SamplingBlasContext(blas_threads):
    ...
    def search_min_by(metric_key: str) -> tuple[int, dict]:
        def evaluate_at_r(r: int) -> dict:
            ...
            if metric_key == "expectation_bias":
                evolution_data = get_evolution_data(static, evolution_cache, cache_key, t_total, r, mode_impl, cache_lock)
                exact_output_batch, deterministic_output_batch = ensure_basis_batches_cached(
                    mode_impl, static, evolution_data, r, cache_lock
                )
...
def run_repetition_worker(args, repetition: int, static: dict, evolution_cache: dict, cache_lock, case_prefix: str, mode_impl):
    searches: list[dict] = []
    ...
    return {
        "repetition": repetition,
        "sampled_r_min": int(sampled_r_min),
        "expected_r_min": -1 if expected_r_min is None else int(expected_r_min),
        "sample_error": float(metrics["sample_error"]),
        "expectation_bias": float(metrics["expectation_bias"]),
        "sample_fluctuation": float(metrics["sample_fluctuation"]),
        "searches": searches,
    }


def run_repetition_inline(
    args,
    repetition: int,
    static: dict,
    payload: dict,
    ...
    sampled_r_min, metrics, expected_r_min = search_r_min(
        ...
        mode_impl=mode_impl,
        cache_lock=EVOLUTION_CACHE_LOCK,
        blas_threads=args.blas_threads if blas_threads is None else blas_threads,
    )
    return {
        "repetition": repetition,
        "sampled_r_min": int(sampled_r_min),
...
def merge_repetition_result(payload: dict, result: dict):
    payload["sampled_r_mins"].append(result["sampled_r_min"])
    payload["expected_r_mins"].append(result["expected_r_min"])
    payload["sample_errors"].append(result["sample_error"])
    payload["expectation_biases"].append(result["expectation_bias"])
    payload["sample_fluctuations"].append(result["sample_fluctuation"])
    payload["searches"].extend(result["searches"])
...
def run_case(args, case_index=None, total_cases=None):
    ...
    q0, s0 = resolved_channel_parameters(args)
    output_path = sampling_output_path(args, q0, s0)
    payload = {
        "script": "NCC_channel_parallel_r.py",
        "mode": args.mode,
        "params": {
            "N": args.N,
            "J": args.J,
            ...
            "blas_threads": args.blas_threads,
            "warmup_blas_threads": args.warmup_blas_threads,
        },
        "sampled_r_mins": [],
        "expected_r_mins": [],
        "sample_errors": [],
    ...
    max_workers = min(args.repeats, os.cpu_count() or 1) if requested_workers <= 0 else min(args.repeats, requested_workers)
    if max_workers <= 1:
        static = build_static_for_case(args, mode_impl, q0, s0)
        for repetition in range(args.repeats):
            result = run_repetition_inline(
                args,
                repetition,
                static,
                payload,
            ...
            ci_low, ci_high = merge_repetition_result(payload, result)
            save_sampling_checkpoint(output_path, payload)
            print(
                f"{case_prefix}[repeat {repetition + 1}/{args.repeats}] sampled_r_min={result['sampled_r_min']}, "
                f"expected_r_min={result['expected_r_min']}, current mean={np.mean(payload['sampled_r_mins']):.3f}, "
                f"95% CI=[{ci_low:.3f}, {ci_high:.3f}]"
            )
    else:
        if args.save_every_search:
            print(f"{case_prefix}save_every_search is ignored while workers={max_workers}; checkpoints are saved per completed repeat.")
        static = build_static_for_case(args, mode_impl, q0, s0)
        warmup_repeats = max(0, min(int(args.warmup_repeats or 0), args.repeats))
        if warmup_repeats:
            print(f"{case_prefix}warming shared evolution cache with {warmup_repeats} serial repeat(s)")
        for repetition in range(warmup_repeats):
            result = run_repetition_inline(
                args,
                repetition,
                static,
                ...
                mode_impl,
                checkpoint_cb=None,
                blas_threads=args.warmup_blas_threads,
            )
            ci_low, ci_high = merge_repetition_result(payload, result)
            save_sampling_checkpoint(output_path, payload)
            print(
                f"{case_prefix}[repeat {repetition + 1}/{args.repeats}] warmup sampled_r_min={result['sampled_r_min']}, "
                f"expected_r_min={result['expected_r_min']}, current mean={np.mean(payload['sampled_r_mins']):.3f}, "
                f"95% CI=[{ci_low:.3f}, {ci_high:.3f}]"
            )

        remaining_repetitions = list(range(warmup_repeats, args.repeats))
        if not remaining_repetitions:
            completed = {}
        else:
            max_workers = min(max_workers, len(remaining_repetitions))
            print(
                f"{case_prefix}running {len(remaining_repetitions)} remaining repeats with "
                f"{max_workers} worker threads sharing warmed evolution cache"
            )
        ...
        if remaining_repetitions:
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(
                        run_repetition_worker,
                        args,
                        repetition,
                        static,
                        EVOLUTION_CACHE,
                        EVOLUTION_CACHE_LOCK,
                        case_prefix,
                        mode_impl,
                    ): repetition
                    for repetition in remaining_repetitions
                }
                for future in concurrent.futures.as_completed(futures):
                    repetition = futures[future]
                    completed[repetition] = future.result()
                    print(f"{case_prefix}[repeat {repetition + 1}/{args.repeats}] completed")

        for repetition in remaining_repetitions:
            result = completed[repetition]
            ci_low, ci_high = merge_repetition_result(payload, result)
            save_sampling_checkpoint(output_path, payload)
            print(
                f"{case_prefix}[repeat {repetition + 1}/{args.repeats}] sampled_r_min={result['sampled_r_min']}, "
