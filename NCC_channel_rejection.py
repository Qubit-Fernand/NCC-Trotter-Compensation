...
def build_rejection_layers(n: int, coupling_j: float, field_h: float):
    """Build the three BCH-input Hamiltonians H1=B, H2=A, H3=-(A+B)."""
    terms_a, terms_b = heisenberg_ab_term_specs(n, coupling_j, field_h)
    terms_neg_total = [(alpha, -cached_pauli_matrix_from_label(label), support_from_label(label)) for alpha, label in terms_a + terms_b]
    ...
    layers = [
        to_layer_records(terms_b, 1, negative=False),
        to_layer_records(terms_a, 2, negative=False),
        [
            {
                "alpha": alpha,
                "matrix": pauli,
            ...
            for gamma, (alpha, pauli, support) in enumerate(terms_neg_total)
        ],
    ]
    return layers
...
def sigma_coefficient(perm):
    descents = sum(perm[i] > perm[i + 1] for i in range(len(perm) - 1))
    return ((-1.0) ** descents) / (len(perm) ** 2 * math.comb(len(perm) - 1, descents))
...
def lc_sample(
    ...
    for depth in range(2, q + 1):
        ...
        if rng.random() > local_weight / (depth * k_local * g):
            return None
...
def bch_sample(
    ...
    res = lc_sample(rng, layers, n, k_local, g, q)
    if res is None:
        return None, 0.0
...
def rejection_raw_entries(q0: int, s0: int, K: int, n: int, k_local: int, g: float, amax_kappa_plus_one: float, x: float):
    """Unnormalized weights from Eq. (27) for the higher-order pseudocode."""
    entries = []
    for s in range(2 * K + 2, s0 + 1):
        max_parts = s // (K + 1)
...
def ncc_sample_rejection(
    ...
    for q in q_tuple[:j_parts]:
        sampled_unitary, sampled_sign = bch_sample(rng, layers, n, k_local, g, q)
...
def exact_tail_action_on_rho(n: int, q0: int, s0: int, coupling_j: float, field_h: float, K: int, x: float, rho: np.ndarray):
    """Exact higher-order tail action used only for small-system validation."""
